  ...
  const methods = {
    ...
    init: async function (options) {
      ...
      if (_data.options.options.apiKey) {
        ...
        setTimeout(() => {
          if (window.Gleap && !window.Gleap.isUserIdentified()) {
            _this.identifyGleap(_data.options.options.apiKey)
      ...
      // START tokens
      _data.tokensRepo = mapsvg.useRepository("tokens")
      _data.tokensRepo.events.on("afterLoad", () => {
        _this.setInputGrantAccessState()
      })
      _data.tokensRepo.events.on("afterCreate", () => {
        _this.loadTokens()
      })
      _data.tokensRepo.events.on("afterDelete", () => {
        _this.loadTokens()
      })
      _data.tokensRepo.find()
      // END
    ...
    initSupportModal: function () {
      ...
      // Clear the tokens table when the modal is hidden
      ...
      $("#mapsvg-toggle-magic-link").on("click", (e) => {
        e.preventDefault()
        $("input[name=accessLogs]").trigger("change")
      })

      $(".mapsvg-copy-input").on("click", function (e) {
        e.preventDefault()
        _this.copyMagicLinkToClipBoard()
      })
      $("#mapsvg-save-api-token").on("click", async () => {
        const apiKey = $("#apiTokenInput").val().trim()

        try {
          await _this.updateChatToken(apiKey)
          customModal.hide()
        } catch (e) {
          console.error(e)
        }
