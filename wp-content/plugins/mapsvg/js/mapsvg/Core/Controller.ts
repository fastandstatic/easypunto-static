...
import { deepMerge, parseBoolean, useId } from "./Utils"
...
export class Controller implements BaseController {
  ...
  setEventHandlers() {
    if (this.closable) {
      $(this.containers.toolbar).on("click touchend", ".mapsvg-popover-close", (e) => {
        e.stopPropagation()
        this.close()
      })
      $(this.containers.view).on("click touchend", ".mapsvg-mobile-modal-close", (e) => {
        e.stopPropagation()
        this.close()
      })
